...
# Making sure user is logged in to correct Azure tenant and subscription
##############################################################################################################################################################################################################################################
Write-Host "--------------------------------------------------------"
Write-Host "|  Checking Azure CLI login status and assigned roles  |"
Write-Host "--------------------------------------------------------"
...
# Configuring settings for the OpenID Connect integration between GitHub and Azure
##############################################################################################################################################################################################################################################
Write-Host "-----------------------------------------------"
...
$userAssignedIdentityClientId = az identity create --name $userAssignedManagedIdentityName --resource-group $resourceGroupName --location $location --subscription $subscriptionId --tags $tags --query clientId -o tsv
...
Write-Host "Configuring federated credentials for user assigned managed identity '$userAssignedManagedIdentityName'"
$audience = "api://AzureADTokenExchange"
$issuer = "https://token.actions.githubusercontent.com"

Write-Host "Create a federated identity credential for '$repositoryName' main branch"
...
Write-Host "Create a federated identity credential for '$repositoryName' pull request"
...
Write-Host "AZURE_CLIENT_ID: '$userAssignedIdentityClientId'"
...
gh secret set AZURE_CLIENT_ID -b $userAssignedIdentityClientId --repo $repositoryName
